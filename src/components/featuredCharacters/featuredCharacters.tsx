...
export function FeaturedCharacters() {
  ...
  return (
    <section className='flex pt-12 pb-24 px-16 flex-wrap gap-4 justify-center bg-primary min-h-96'>
      ...
      {isLoading && Array.from({ length: 4 }).map((_, index) => <CharacterCard.Skeleton key={index} />)}
      {isError && <p className='text-2xl text-white text-center w-full'>Something went wrong. Try again later.</p>}
